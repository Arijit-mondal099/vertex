...
if (!existsSync(CACHE_PATH)) {
  console.error(
    "videos.json is missing. Run: node scripts/seed/resolve-videos.mjs",
  );
  process.exit(1);
}
...
const coverImageUrl = (slug) =>
  `https://picsum.photos/seed/vertex-${slug}/1600/900`;
const posterUrl = (videoId) =>
  `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`;
const portraitUrl = ({ gender, id }) =>
...
for (const category of categories) {
  check(
    ...
    `category ${category.slug}: description over 200 chars`,
  );
  documents.push({
...
for (const instructor of instructors) {
  ...
  documents.push({
    ...
    photo: image(
      portraitUrl(instructor.portrait),
      `Portrait of ${instructor.name}`,
    ),
...
for (const course of courses) {
  check(
    categorySlugs.has(course.category),
    `course ${course.slug}: unknown category`,
  );
  check(
    instructorSlugs.has(course.instructor),
    `course ${course.slug}: unknown instructor`,
  );
  check(
    course.summary.length <= 200,
    `course ${course.slug}: summary over 200 chars`,
  );
  check(
    course.outcomes.length <= 6,
    `course ${course.slug}: more than 6 learning outcomes`,
  );
  check(
    course.modules.length >= 1,
    `course ${course.slug}: needs at least one module`,
  );
  ...
  const modules = course.modules.map((module, moduleIndex) => {
    ...
    const lessonRefs = module.lessons.map((lesson) => {
      ...
      check(
        Boolean(video),
        `lesson ${slug}: no resolved video. Re-run resolve-videos.mjs`,
      );
      ...
      if (video) {
        ...
        check(
          !owner,
          `lesson ${slug}: video ${video.id} already used by ${owner}`,
        );
        usedVideoIds.set(video.id, slug);
      }

      check(
        lesson.points.length <= 6,
        `lesson ${slug}: more than 6 key points`,
      );
      check(
        !lesson.proTip || lesson.proTip.length <= 280,
        `lesson ${slug}: pro tip over 280 chars`,
      );
      ...
      const studentCount = Math.round(
        course.studentCount *
          (1 - (position / Math.max(totalLessons, 1)) * 0.62),
      );
      ...
      const resources = [
        {
          ...
          description:
            "The official reference for the tools used in this lesson.",
          url: course.docs.url,
        },
      ];
      if (lesson.resource) {
        check(
          !lesson.resource.description ||
            lesson.resource.description.length <= 160,
          `lesson ${slug}: resource description over 160 chars`,
        );
        resources.push({
          _type: "resource",
          _key: keyOf(slug, "extra"),
          ...lesson.resource,
        });
      }
      ...
      documents.push({
        ...
        thumbnail: image(
          posterUrl(video?.id ?? "missing"),
          `Video thumbnail for ${lesson.title}`,
        ),
  ...
  documents.push({
    ...
    coverImage: image(
      coverImageUrl(course.slug),
      `Cover image for ${course.title}`,
    ),
    ...
    learningOutcomes: course.outcomes.map((outcome, index) => {
      check(
        outcome.title.length <= 60,
        `course ${course.slug}: outcome title over 60 chars`,
      );
      check(
      ...
      return {
        ...
        _key: keyOf(course.slug, "outcome", String(index)),
        ...outcome,
      };
    }),
    modules,
...
for (const id of lessonIds) {
  check(
    referencedLessonIds.has(id),
    `lesson ${id} is orphaned: no course references it`,
  );
}
...
if (problems.length) {
  console.error(
    `Refusing to write ${OUT_PATH}. ${problems.length} problem(s):\n`,
  );
