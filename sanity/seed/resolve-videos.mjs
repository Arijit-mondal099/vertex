...
async function search(phrase) {
  ...
  if (!response.ok)
    throw new Error(`Search failed (${response.status}) for "${phrase}"`);
...
for (const [index, lesson] of pending.entries()) {
  ...
  if (chosen) {
    ...
    resolved += 1;
    console.log(
      `${label} — ${chosen.id} (${chosen.duration}s) ${chosen.title}`,
    );
...
if (failures.length) {
  ...
  console.warn(
    "Re-run to retry them, or adjust the search phrase in content.mjs.",
  );
  process.exitCode = 1;
}
